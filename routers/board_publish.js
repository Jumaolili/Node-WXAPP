...
router.post('/board_publish', function(req, res) {
    //1. 接受客户端传输的数据
    ...
    var promise = new Promise(function(resolve, reject) {
        if (picture_base64) {
            ...
            var url = './user_src/board_img/' + user_id + '_' + (Math.random().toString().split('.')[1]) + '.jpg';
    ...
    promise
        .then(function(url) {
            ...
            return new Promise(function(resolve, reject) {
                new Boards(item).save(function(err) {
                    if (err) {
                        reject(err)
                    } else {
                        resolve(item);
